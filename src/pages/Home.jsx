...
function Home() {
  ...
  const fotosGaleria = useMemo(() => {
    return galerias.flatMap((grupo) => {
      const imagenes = Array.isArray(grupo.imagenes)
        ? grupo.imagenes
        : [];
  ...
  useEffect(() => {
    if (fotosGaleria.length <= FOTOS_VISIBLES) {
      return;
    }

    const intervalo = setInterval(() => {
      setIndiceGaleria((actual) => {
        const siguiente =
          actual + FOTOS_VISIBLES;

        if (siguiente >= fotosGaleria.length) {
          return 0;
        }
  ...
  const proyectosVisibles = useMemo(() => {
    ...
    let visibles = proyectos.slice(
      indiceProyecto,
      indiceProyecto + PROYECTOS_VISIBLES
    );
  ...
  const fotosVisibles = useMemo(() => {
    ...
    let visibles = fotosGaleria.slice(
      indiceGaleria,
      indiceGaleria + FOTOS_VISIBLES
    );

    if (visibles.length < FOTOS_VISIBLES) {
      visibles = [
        ...visibles,
        ...fotosGaleria.slice(
          0,
          FOTOS_VISIBLES - visibles.length
        ),
      ];
    }
  ...
  const proyectosAnteriores = () => {
    ...
    setIndiceProyecto((actual) => {
      ...
      if (anterior < 0) {
        ...
        return (
          ultimaPagina * PROYECTOS_VISIBLES
        );
      }
        ...
        >
          ...
          <div className="max-w-3xl">
            ...
            <p className="text-zinc-300 text-base md:text-lg leading-relaxed max-w-2xl mt-5">
              En Wealth integramos construcción, ingeniería,
              desarrollo inmobiliario y soluciones en aluminio y
              vidrio para transformar ideas en proyectos funcionales,
        ...
        <section className="pt-10 pb-8">
          ...
          <div className="mb-6">
            ...
            <p className="text-zinc-500 mt-2 max-w-2xl">
              Soluciones integrales para desarrollar proyectos desde
            ...
            >
              ...
              {fotosVisibles.map((foto) => (
                ...
                >
                  ...
                  <img
                    src={foto.imagen}
                    alt={foto.subcategoria || "Galería Wealth"}
                    loading="lazy"
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                  />

                  <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition" />

                  <div className="absolute left-3 right-3 bottom-3 opacity-0 group-hover:opacity-100 transition">
